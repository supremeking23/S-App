...
if (isset($_GET['delete_tutorial_id'])) {
 
   $department_id =  $_GET['department_id'];
 
  $tutorial_id = mysql_prep($_GET['delete_tutorial_id']);
  $query = "DELETE FROM tbltutorials WHERE tutorial_id = '$tutorial_id' ";
  
  $delete_data = mysqli_query($connection,$query)or die(mysqli_error($connection));

    if($delete_data && mysqli_affected_rows($connection) == 1){
        $_SESSION['success_message'] = "Tutorial has been deleted";
                redirect_to('../department_info.php?department_id=' .$department_id);
      }
